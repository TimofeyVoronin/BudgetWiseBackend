...
Security-диагностики имеют приоритет над семантическими ошибками неизвестной функции или переменной. Например, для `eval()` возвращается ошибка безопасности, а не дополнительная ошибка `unknown-function`.
